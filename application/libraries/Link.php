...
class LINK {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = LINK::pluck( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = LINK::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
     ...
     * in response to an LINK request, or can be modified if needed before
    ...
    {
        $bindings = array();
        $db = LINK::sql_connect( $sql_details );

        // Build the SQL query string from the request
        $limit = LINK::limit( $request, $columns );
        $order = LINK::order( $request, $columns, $joinQuery );
        $where = LINK::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            $col = LINK::pluck($columns, 'db', $joinQuery);
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", LINK::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
        ...
        $data = LINK::sql_exec( $db, $bindings,$query);

        // Data set length after filtering
        $resFilterLength = LINK::sql_exec( $db,
            "SELECT FOUND_ROWS()"
        );
        ...
        $resTotalLength = LINK::sql_exec( $db, $total_query);
        ...
        return array(
            ...
            "data"            => LINK::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            LINK::fatal( "An SQL error occurred: ".$e->getMessage() );
        }
